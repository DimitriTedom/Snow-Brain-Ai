  ...
  return (
    <textarea
      className={cn(
        ...
        // Light theme styles
        'border-gray-400 bg-white text-gray-900 placeholder:text-gray-500 ring-offset-white focus-visible:ring-gray-600',
        // Dark theme styles  
        'dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-400 dark:ring-offset-gray-800 dark:focus-visible:ring-gray-400',
        className,
      )}
      ref={ref}
      {...props}
      ...
      {suggestedActions.map((suggestedAction, index) => (
        ...
        >
          <Button
            ...
            className="text-left border rounded-2xl px-6 py-4 text-base flex-1 gap-2 sm:flex-col w-full h-auto justify-start items-start
                       border-gray-300/60 dark:border-white/20 
                       ...
                       hover:shadow-2xl"
          >
            <span className="font-semibold text-blue-600 dark:text-blue-300">{suggestedAction.title}</span>
...
function PureAttachmentsButton({
  ...
  return (
    <Button
      ...
      className="rounded-xl p-3 h-fit border border-gray-300/50 dark:border-white/20 
                 hover:bg-gray-200/50 dark:hover:bg-white/20 backdrop-blur-xl 
...
function PureStopButton({ onStop }: { onStop: () => void }) {
  return (
    <Button
      ...
      className="rounded-full p-3 h-fit bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white shadow-xl transition-all duration-300 hover:scale-110"
      onClick={(event) => {
        event.preventDefault();
        onStop();
      }}
...
function PureSendButton({
  ...
  return (
    <Button
      ...
      className="rounded-full p-3 h-fit bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white shadow-xl transition-all duration-300 hover:scale-110 disabled:opacity-50 disabled:hover:scale-100"
      onClick={(event) => {
        event.preventDefault();
        if (!isDisabled) {
          submitForm();
  ...
  return (
    <div className={cn("relative w-full flex flex-col gap-4", className)}>
      ...
      <Textarea
        ...
        ref={textareaRef}
        placeholder="Type your message to Snow Brain AI..."
        value={input}
        onChange={handleInput}
        className={cn(
          'min-h-[60px] max-h-[calc(40vh)] overflow-y-auto resize-none rounded-2xl !text-lg pb-12',
          'bg-white/10 dark:bg-white/5 border border-white/30 dark:border-white/20 backdrop-blur-xl', 
          'text-gray-900 dark:text-white placeholder:text-gray-600 dark:placeholder:text-white/50',
